...
can=[1 for i in range(0,l)] # one candy to every student




"""
This was my solution that dont work as each students candy depends on both its prev as well as ahead student.
The array going forward accounts for previous but not ahead>> we need to run a reverse array 
if A[0]>A[1]:
    can[0]=2
...
for i in range(l-2,-1,-1):
    if (A[i]>A[i+1])  and can[i]<=can[i+1]: # **I dont understand why this AND can[i]<=... is added here or how it works 
       can[i]=can[i+1]+1
